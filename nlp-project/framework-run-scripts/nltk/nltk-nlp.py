	...
	for sent in _sents:
		...
		for pos in _pos:
			...
			if tag != '':
				lem = '{}/{}/{}'.format(pos[0],
						pos[1],
						wordnet_lemmatizer.lemmatize(pos[0], tag))
			else:
				lem = '{}/{}/{}'.format(pos[0],
						pos[1],
						wordnet_lemmatizer.lemmatize(pos[0]))
			_lemmas.append(lem)
	return _lemmas
		...
		if type(node) is nltk.Tree:
			if node.label() == SENTENCE:
				...
				for leaf in node.leaves():
					ner = '{}/{}/{}'.format(leaf[0], leaf[1], node.label())
					current = leaf[0]
					_ners.append(ner)
			ner_process(node)
		else:
			if first == 1:
				ner = '{}/{}'.format(node[0], node[1])
				first = 0 
				_ners.append(ner)
			elif node[0] != current:
				print node[0], ' and last ', _ners[-1]
				ner = '{}/{}'.format(node[0], node[1])
				_ners.append(ner)
			else:
				pass
